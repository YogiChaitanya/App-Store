...
class AppStore extends Component {
  state = {
    activeTabId: appsList[0].appId,
    searchInput: '',
    appDetailsList: appsList,
  }

  // method 1
  // which tab is selected tab
  ...
  // method 1
  // based on tab selctions we want results
  getFilteredApps = () => {
  ...
  // method 2
  // based on search input we want render the results
  ...
  render() {
    const {searchInput, activeTabId, appDetailsList} = this.state

    // method 1 for getting results
    let filteredApps = this.getFilteredApps()
    // method 2 for getting results
    filteredApps = appDetailsList.filter(eachApp =>
